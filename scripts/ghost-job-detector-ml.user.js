// ==UserScript==
// @name         Ghost Job Detector - ML
// @namespace    https://github.com/ghost-job-detector
// @version      2.0.0
// @description  ML-powered ghost job detection with custom model support
// @author       Anonymous
...
// ml version - train your own model to detect ghost jobs
// extracts features from job listings and runs them through tensorflow
// see training folder for how to build a model
...
(function() {
    ...
    var cfg = {
        ...
        mlThresh: GM_getValue('gjd_mlthresh', 0.6),
        hl: GM_getValue('gjd_hl', true),
        scores: GM_getValue('gjd_scores', true),
        reasons: GM_getValue('gjd_reasons', true),
        useML: GM_getValue('gjd_useml', true),
        mlUrl: GM_getValue('gjd_mlurl', ''),
    ...
    // ========== HEURISTIC RULES ==========
    var rules = {
        age: { over90: 30, over60: 20, over30: 10, repost: 15 },
        apps: { over1k: 25, over500: 20, over200: 10 },
        sal: { none: 15, huge: 15, comp: 10 },
        co: { staff: 10, client: 15 },
        reqs: { entryExp: 25 },
        flags: { fast: 5, fam: 10, hats: 10, self: 5, pto: 5, ground: 10, rock: 10, hustle: 10, urgent: 15 },
        good: { sal: -10, recent: -10, fewApps: -5, mgr: -10, ben: -5 }
    };

    var staffing = ['robert half', 'randstad', 'adecco', 'manpower', 'apex systems', 'tek systems', 'insight global', 'cybercoders', 'kforce', 'staffing', 'recruiting'];

    var flagPats = [
        { rx: /fast[- ]?paced/i, k: 'fast' },
        { rx: /like a family/i, k: 'fam' },
        { rx: /wear many hats/i, k: 'hats' },
        { rx: /self[- ]?starter/i, k: 'self' },
        { rx: /unlimited (pto|vacation)/i, k: 'pto' },
        { rx: /ground[- ]?floor/i, k: 'ground' },
        { rx: /rock\s?star|ninja/i, k: 'rock' },
        { rx: /hustle|grind/i, k: 'hustle' },
        { rx: /urgent(ly)? (hiring|need)/i, k: 'urgent' }
    ];

    // ========== ML MODEL ==========

    async function loadModel() {
        if (!cfg.mlUrl || !cfg.useML) return;
        if (model || loading) return;

        loading = true;
        upMLStatus('loading', 'Loading...');

        try {
            model = await tf.loadLayersModel(cfg.mlUrl);
            upMLStatus('ok', 'Model loaded');
            console.log('[GJD ML] model loaded');
        } catch(e) {
            model = null;
            upMLStatus('err', 'Failed: ' + e.message.substring(0, 30));
            console.log('[GJD ML] load error', e);
        }
        loading = false;
    }

    function upMLStatus(t, msg) {
        var el = document.getElementById('gjd-ml-st');
        if (!el) return;
        el.className = 'gjd-ml-st ' + t;
        el.textContent = msg;
    }
    ...
    function extractFeatures(c) {
        ...
        // age (0-1)
        var age = getAge(c);
        f.push(age >= 0 ? Math.min(age / 120, 1) : 0.5);

        // applicants (0-1)
        var apps = getApps(c);
        f.push(apps >= 0 ? Math.min(apps / 2000, 1) : 0.3);

        // has salary (0/1)
        f.push(hasSal(c) ? 0 : 1);

        // text length
        f.push(Math.min(txt.length / 5000, 1));

        // red flag count
        var rf = 0;
        for (var i = 0; i < flagPats.length; i++) if (flagPats[i].rx.test(txt)) rf++;
        f.push(rf / 9);

        // staffing agency (0/1)
        var isStaff = 0;
        for (var j = 0; j < staffing.length; j++) if (txt.includes(staffing[j])) { isStaff = 1; break; }
        f.push(isStaff);

        // repost (0/1)
        f.push(/repost/i.test(txt) ? 1 : 0);

        // entry + high exp (0/1)
        var entryHigh = 0;
        if (/entry[- ]?level|junior/i.test(txt)) {
            var m = txt.match(/(\d+)\+?\s*years?/i);
            if (m && parseInt(m[1]) >= 5) entryHigh = 1;
        }
        f.push(entryHigh);

        // hiring for client (0/1)
        f.push(/hiring for (a )?client|confidential/i.test(txt) ? 1 : 0);

        // has benefits (0/1 inverted)
        f.push(/401k|health insurance|dental/i.test(txt) ? 0 : 1);

        // has manager mentioned (0/1 inverted)
        f.push(/hiring manager|reporting to/i.test(txt) ? 0 : 1);

        // competitive salary (0/1)
        f.push(/competitive (salary|pay)/i.test(txt) ? 1 : 0);

        return f;
    }

    async function predictML(c) {
        if (!model) return null;

        try {
            var f = extractFeatures(c);
            var tensor = tf.tensor2d([f]);
            var pred = await model.predict(tensor).data();
            tensor.dispose();
            return pred[0];
        } catch(e) {
            console.log('[GJD ML] predict error', e);
            return null;
        }
    }
    ...
    function getSal(c) {
        ...
        if (m) {
            ...
            if (lo >= 20000) return { lo: lo, hi: hi, range: hi - lo };
        }
        return null;
    }

    function getCo(c) {
        var sels = ['.job-card-container__company-name', '.company-name', '[class*="company"]', '.employer-name'];
        for (var i = 0; i < sels.length; i++) {
            var el = c.querySelector(sels[i]);
            if (el) { var n = el.textContent.trim(); if (n && n.length > 1 && n.length < 100) return n; }
        }
        return null;
    }
    ...
    function heuristic(c) {
        ...
        // lists
        if (co) {
            for (var w = 0; w < cfg.wl.length; w++) if (coLo.includes(cfg.wl[w].toLowerCase())) return { sc: 0, whys: [], co: co, wl: true, ghost: false };
            for (var b = 0; b < cfg.bl.length; b++) if (coLo.includes(cfg.bl[b].toLowerCase())) return { sc: 100, whys: ['blacklisted'], co: co, ghost: true };
        }

        var age = getAge(c);
        if (age >= 90) { pts += rules.age.over90; whys.push('90+ days'); }
        else if (age >= 60) { pts += rules.age.over60; whys.push('60+ days'); }
        else if (age >= 30) { pts += rules.age.over30; whys.push('30+ days'); }
        else if (age >= 0 && age < 7) pts += rules.good.recent;

        if (/repost/i.test(txt)) { pts += rules.age.repost; whys.push('reposted'); }

        var apps = getApps(c);
        if (apps >= 1000) { pts += rules.apps.over1k; whys.push('1k+ apps'); }
        else if (apps >= 500) { pts += rules.apps.over500; whys.push('500+ apps'); }
        else if (apps >= 200) { pts += rules.apps.over200; whys.push('200+ apps'); }
        else if (apps >= 0 && apps < 50) pts += rules.good.fewApps;

        var sal = getSal(c);
        if (!sal) { pts += rules.sal.none; whys.push('no salary'); }
        else { pts += rules.good.sal; if (sal.range >= 50000) { pts += rules.sal.huge; whys.push('huge range'); } }

        if (/competitive (salary|pay)/i.test(txt)) { pts += rules.sal.comp; whys.push('"competitive"'); }

        for (var s = 0; s < staffing.length; s++) {
            if (coLo.includes(staffing[s]) || txt.includes(staffing[s])) { pts += rules.co.staff; whys.push('staffing'); break; }
        }

        if (/hiring for (a )?client|confidential/i.test(txt)) { pts += rules.co.client; whys.push('"for client"'); }

        if (/entry[- ]?level|junior/i.test(txt)) {
            var ym = txt.match(/(\d+)\+?\s*years?/i);
            if (ym && parseInt(ym[1]) >= 5) { pts += rules.reqs.entryExp; whys.push('entry wants 5+ yrs'); }
        }

        for (var f = 0; f < flagPats.length; f++) {
            if (flagPats[f].rx.test(txt)) { pts += rules.flags[flagPats[f].k]; whys.push(flagPats[f].k); }
        }

        if (/hiring manager|reporting to/i.test(txt)) pts += rules.good.mgr;
        if (/401k|health insurance|dental/i.test(txt)) pts += rules.good.ben;
    ...
    async function analyze(c) {
        var h = heuristic(c);
        if (h.wl || h.sc === 100) return h;

        if (cfg.useML && model) {
            var mlP = await predictML(c);
            if (mlP !== null) {
                if (mlP >= cfg.mlThresh) {
                    h.sc += 40;
                    h.whys.push('ML:' + Math.round(mlP * 100) + '%');
                } else if (mlP < 0.3) {
                    h.sc -= 15;
                }
                h.sc = Math.max(0, Math.min(100, h.sc));
                h.ghost = h.sc >= cfg.thresh;
            }
        }
    ...
    function buildUI() {
        ...
        document.getElementById('gjd-ml').onchange = function() { cfg.useML = this.checked; save(); };
        ...
        document.getElementById('gjd-mlurl').onchange = function() { cfg.mlUrl = this.value.trim(); save(); };
        document.getElementById('gjd-load').onclick = loadModel;
    ...
    function save() {
        ...
        GM_setValue('gjd_mlthresh', cfg.mlThresh);
        GM_setValue('gjd_hl', cfg.hl);
        GM_setValue('gjd_scores', cfg.scores);
        GM_setValue('gjd_useml', cfg.useML);
        GM_setValue('gjd_mlurl', cfg.mlUrl);
    ...
    function addWl(n) { if (!n) return false; n = n.trim().toLowerCase(); if (cfg.wl.indexOf(n) === -1) { cfg.wl.push(n); var bi = cfg.bl.indexOf(n); if (bi !== -1) cfg.bl.splice(bi, 1); save(); return true; } return false; }
    function addBl(n) { if (!n) return false; n = n.trim().toLowerCase(); if (cfg.bl.indexOf(n) === -1) { cfg.bl.push(n); var wi = cfg.wl.indexOf(n); if (wi !== -1) cfg.wl.splice(wi, 1); save(); return true; } return false; }

    function showMng() {
        var old = document.getElementById('gjd-modal'); if (old) old.remove();
        ...
        var wlH = cfg.wl.length === 0 ? '<p style="color:#666;text-align:center;padding:10px;">None</p>' : cfg.wl.map(function(x) { return '<div style="display:flex;justify-content:space-between;padding:4px 8px;background:rgba(34,197,94,0.1);border-radius:4px;margin:2px 0;"><span style="font-size:11px;">' + x + '</span><button data-c="' + x + '" data-l="w" class="gjd-rm" style="background:#ef4444;border:none;color:#fff;padding:2px 6px;border-radius:3px;cursor:pointer;font-size:9px;">x</button></div>'; }).join('');
        var blH = cfg.bl.length === 0 ? '<p style="color:#666;text-align:center;padding:10px;">None</p>' : cfg.bl.map(function(x) { return '<div style="display:flex;justify-content:space-between;padding:4px 8px;background:rgba(239,68,68,0.1);border-radius:4px;margin:2px 0;"><span style="font-size:11px;">' + x + '</span><button data-c="' + x + '" data-l="b" class="gjd-rm" style="background:#22c55e;border:none;color:#fff;padding:2px 6px;border-radius:3px;cursor:pointer;font-size:9px;">ok</button></div>'; }).join('');
        ...
        for (var i = 0; i < rms.length; i++) { rms[i].onclick = function() { var c = this.getAttribute('data-c'); var l = this.getAttribute('data-l'); if (l === 'w') { var idx = cfg.wl.indexOf(c); if (idx !== -1) cfg.wl.splice(idx, 1); } else { var idx2 = cfg.bl.indexOf(c); if (idx2 !== -1) cfg.bl.splice(idx2, 1); } save(); modal.remove(); showMng(); scan(); }; }
        document.getElementById('gjd-done').onclick = function() { modal.remove(); };
        modal.onclick = function(e) { if (e.target === modal) modal.remove(); };
    }

    function doExp() { var d = { wl: cfg.wl, bl: cfg.bl, thresh: cfg.thresh, mlUrl: cfg.mlUrl }; var blob = new Blob([JSON.stringify(d, null, 2)], { type: 'application/json' }); var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'ghost-detector-ml.json'; a.click(); }

    function doImp() {
        var inp = document.createElement('input'); inp.type = 'file'; inp.accept = '.json';
        inp.onchange = function(e) { var f = e.target.files[0]; if (!f) return; var r = new FileReader(); r.onload = function(ev) { try { var d = JSON.parse(ev.target.result); if (d.wl) for (var i = 0; i < d.wl.length; i++) addWl(d.wl[i]); if (d.bl) for (var j = 0; j < d.bl.length; j++) addBl(d.bl[j]); if (d.mlUrl) { cfg.mlUrl = d.mlUrl; document.getElementById('gjd-mlurl').value = d.mlUrl; } save(); scan(); alert('Imported!'); } catch(err) { alert('Error: ' + err.message); } }; r.readAsText(f); };
        inp.click();
    }
    ...
    function findCards() {
        ...
        var cards = [], seen = [];
        for (var i = 0; i < sels.length; i++) { var els = document.querySelectorAll(sels[i]); for (var j = 0; j < els.length; j++) { if (seen.indexOf(els[j]) === -1 && els[j].offsetHeight > 50) { cards.push(els[j]); seen.push(els[j]); } } }
        return cards;
    }

    async function proc(c) {
        var res = await analyze(c);
        ...
        if (res.ghost) {
            ...
            if (cfg.hl) {
                ...
                if (cfg.reasons && res.whys.length) { var why = document.createElement('div'); why.className = 'gjd-why'; why.innerHTML = res.whys.slice(0, 4).join('<br>'); c.appendChild(why); }
                addActs(c, res);
            } else c.classList.add('gjd-hide');
        } else if (res.sc >= 30) {
            stats.sus++;
            if (cfg.hl) { c.classList.add('gjd-sus'); if (cfg.scores) { var sc = document.createElement('div'); sc.className = 'gjd-score md'; sc.textContent = res.sc + '%'; c.appendChild(sc); } addActs(c, res); }
        } else {
            stats.ok++;
    ...
    async function scan() {
        ...
        for (var k = 0; k < cards.length; k++) await proc(cards[k]);
    ...
    function init() {
        console.log('[Ghost Detector ML] v' + VER);
        buildUI();
        if (cfg.mlUrl && cfg.useML) loadModel();
